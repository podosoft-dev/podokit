...
export interface PodokitManifest {
  ...
  /** Active implementation for each provider-neutral runtime capability. */
...
export const DEFAULT_OWNED_GLOBS = [
  ...
  // Owned Elysia extension slot — register application services and plugins here.
...
export function readManifest(projectRoot: string): PodokitManifest | null {
  ...
  const raw = JSON.parse(readFileSync(file, "utf8")) as Partial<PodokitManifest>;
  if (
    (raw.schemaVersion !== 3 && raw.schemaVersion !== LOCK_SCHEMA_VERSION)
    || raw.toolchain?.runtime !== "bun"
    || raw.toolchain.packageManager !== "bun"
  ) {
    throw new Error(
      "This project was generated by PodoKit v0 and is not compatible with PodoKit v1 updates. Use @podosoft/podokit@0.17.4 for this project; create a new v1 project for Bun and Elysia.",
    );
  }
  const providers = resolveProviderSelections(
    raw.providers as Partial<Record<keyof ProviderSelections, string>> | undefined,
  );
  return { ...raw, schemaVersion: LOCK_SCHEMA_VERSION, providers } as PodokitManifest;
}
...
/** Complete replay variables for a Bun-only v1 project. */
export function manifestTemplateVars(manifest: PodokitManifest): TemplateVars {
  return {
    ...manifest.answers,
    ...providerTemplateVars(manifest.providers),
    ...toolchainTemplateVars(manifest.toolchain),
  };
}

export function providerTemplateVars(providers: ProviderSelections): TemplateVars {
  return {
    databaseProvider: providers.database,
    cacheProvider: providers.cache,
    objectStorageProvider: providers["object-storage"],
    eventsProvider: providers.events,
    jobsProvider: providers.jobs,
  };
}
...
export interface InitLockOptions {
  ...
  version?: string;
  ownedGlobs?: string[];
  providers?: Partial<Record<keyof ProviderSelections, string>>;
}
...
export function initLockfile(projectRoot: string, options: InitLockOptions): void {
  ...
  const providers = resolveProviderSelections(options.providers ?? SERVER_PROVIDER_SELECTIONS);
  const manifest: PodokitManifest = {
    schemaVersion: LOCK_SCHEMA_VERSION,
    podokitVersion: options.version ?? podokitVersion(),
    template: options.template,
    toolchain: options.toolchain ?? resolveToolchain(),
