...
describe("addModule (auth / better-auth)", () => {
  ...
  it("admin-dashboard composes auth and overlays the dashboard UI + admin plugin", () => {
    ...
    expect(setupTwoFactorPage).toContain('method: "totp"');
    expect(setupTwoFactorPage).toContain('data?.method !== "totp"');
    ...
    expect(accountPage).toContain('method: "totp"');
    expect(accountPage).toContain('res?.method !== "totp"');
