...
async function signedIn(
  ...
  const context = await playwright.request.newContext({ baseURL: base, extraHTTPHeaders: { origin: base } });
  if (create) {
    const signUp = await context.post("/api/auth/sign-up/email", { data: account });
...
test("public posts and comments use paginated envelopes @smoke", async ({ request }) => {
...
test("authenticated author can publish, edit, comment, and delete", async ({ playwright }) => {
  const marker = Date.now();
  const context = await signedIn(
    playwright,
    { name: "Blog Author", email: `blog-author-${marker}@example.com`, password: USER.password },
    true,
  );
  const created = await context.post("/api/blog", {
    data: { title: `User post ${marker}`, excerpt: "Published immediately", body: "# Safe", tags: ["e2e"] },
  });
  ...
  const comment = await context.post(`/api/blog/${post.slug}/comments`, { data: { body: "A comment" } });
  expect(comment.ok()).toBeTruthy();
  const commentBody = await comment.json();
  const editedComment = await context.patch(`/api/blog/comments/${commentBody.id}`, {
    data: { body: "An edited comment" },
  });
  expect(editedComment.ok()).toBeTruthy();

  const updated = await context.patch(`/api/blog/${post.id}`, { data: { excerpt: "Updated" } });
...
test("another user cannot modify an author's post while an admin can", async ({ playwright }) => {
  const marker = Date.now();
  const user = await signedIn(
    playwright,
    { name: "Post Owner", email: `blog-owner-${marker}@example.com`, password: USER.password },
    true,
  );
  ...
  const other = await signedIn(
    playwright,
    { name: "Other User", email: `blog-other-${marker}@example.com`, password: USER.password },
    true,
  );
  const forbiddenUpdate = await other.patch(`/api/blog/${post.id}`, { data: { excerpt: "Not mine" } });
  ...
  const updated = await admin.patch(`/api/admin/blog/${post.id}`, { data: { status: "draft" } });
  expect(updated.ok()).toBeTruthy();
...
test("blog validation rejects oversized page requests and empty comments", async ({ playwright, request }) => {
  ...
  if (first?.slug) {
    expect((await user.post(`/api/blog/${first.slug}/comments`, { data: { body: "" } })).status()).toBe(400);
  }
  await user.dispose();
});
